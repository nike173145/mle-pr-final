...
Приведённые ниже метрики и приложенный артефакт относятся к ранее выполненному
1%-му прогону, а не к полному датасету. В этом прогоне модель проверена на
последнем полностью размеченном переходе апрель→май 2016 и улучшила основную
метрику `MAP@7` относительно popularity baseline на 24.5%, увеличив покрытие
каталога с 75.0% до 87.5%. После перехода на обработку всех клиентов результаты
нужно пересчитать командой `bash scripts/train.sh`.
...
Метрики прежнего прогона считаются по всем его 9 353 клиентам holdout, включая
96.89% клиентов без новой покупки. Для них AP, precision и recall равны нулю.
Поэтому небольшие абсолютные значения ожидаемы для настолько разреженной задачи.
Источник сохранённых значений —
[`reports/model_comparison.json`](reports/model_comparison.json), параметры
артефакта — [`reports/model_metadata.json`](reports/model_metadata.json).
...
наличие продуктов. Сводка полного набора зафиксирована в
...
Исходный CSV весит около 2.29 GB. `read_snapshots(DATA_PATH)` загружает его
целиком одним вызовом `pandas.read_csv`, после чего нормализует типы, даты и
продуктовые флаги. В обработке участвуют все клиенты и все их доступные месяцы;
chunking и выборка строк или клиентов не используются. Для запуска требуется
машина с достаточным объёмом RAM: представление DataFrame в памяти занимает
больше места, чем CSV на диске.
...
Текущий артефакт в [`ml_models/model.joblib`](ml_models/model.joblib) и
сохранённые отчёты пока отражают прежний 1%-й прогон: 137 867 снимков,
118 790 train-пар и 9 353 holdout-пары; после оценки refit использовал
128 143 пары. Код загрузки теперь обрабатывает полный CSV без выборки, но
артефакт ещё не переобучен. Для получения результатов по всем клиентам выполните
`bash scripts/train.sh`; команда заново создаст модель и отчёты.
...
Оба ноутбука вызывают `read_snapshots(DATA_PATH)`, загружают полный CSV в память
одним чтением и обрабатывают всех клиентов. Перед запуском убедитесь, что машине
достаточно RAM. Сохранённые outputs очищены после изменения загрузчика, поэтому
ноутбуки необходимо выполнить заново.
...
Обе команды всегда загружают и обрабатывают полный набор клиентов. Это требует
существенно больше RAM и времени, чем прежний демонстрационный прогон.
...
- Приложенный артефакт и отчёты пока относятся к прежнему 1%-му прогону; три
  крайне редких продукта не дали положительных train-событий. Их нужно
  пересоздать командой `bash scripts/train.sh`, чтобы получить full-data
  результаты и снизить этот cold-label риск.
