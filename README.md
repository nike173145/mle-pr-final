...
`02_modeling.ipynb` использует `read_snapshots(DATA_PATH)`; его результаты
очищены, поэтому для просмотра экспериментов ноутбук нужно выполнить заново.
Перед запуском убедитесь, что машине достаточно RAM.
