...
Команда `bash scripts/train.sh` обучает модель на полном наборе данных и сохраняет
сравнение экспериментов в
...
Команда `bash scripts/train.sh` загружает полный CSV, обучает модель на всех
доступных клиентах и сохраняет готовый артефакт в
[`ml_models/model.joblib`](ml_models/model.joblib). Вместе с моделью команда
заново создаёт отчёты с параметрами и метриками.
