...
До сборки должен существовать `ml_models/model.joblib`. Приложенный файл пока
относится к прежнему 1%-му прогону. Чтобы перед запуском сервиса получить
full-data артефакт, выполните `bash scripts/train.sh`: обучение загрузит полный
CSV одним вызовом `pandas.read_csv` и обработает всех клиентов, поэтому машине
потребуется достаточный объём RAM. Затем создайте `.env` и заполните подключение
к внешним сервисам:
